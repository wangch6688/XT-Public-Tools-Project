...
- (CGFloat)setContentView:(NSArray *)contenArr titleArr:(NSArray *)titleArr{
    ...
    //设置滚动视图的滚动范围
    CGFloat contenViewHeight = self.frameRect.size.height + self.frameRect.origin.y + 10;
    self.scroller.contentSize = CGSizeMake(self.width,  self.frameRect.size.height + self.frameRect.origin.y + 10);
    if (_isScrollView) {
        return self.height;
    }else{
        if (self.isShowNoAddCount) {
            self.scroller.frame = CGRectMake(self.scroller.frame.origin.x, self.scroller.frame.origin.y, self.scroller.frame.size.width, self.frame.size.height);
        } else {
            self.scroller.frame = CGRectMake(self.scroller.frame.origin.x, self.scroller.frame.origin.y, self.scroller.frame.size.width, contenViewHeight);
        }
        
        return contenViewHeight;
    }
}
...
- (CGRect)setSignView:(NSArray *)dataAr andTitle:(NSString *)titleStr andFrame:(CGRect)frame andGroupId:(NSInteger)groupId{
    ...
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(_maragin, frame.size.height + frame.origin.y + 10, 0, _titleLabHeight)];
    ...
    CGFloat butorignX = _maragin;
    CGFloat alineButWidth = _maragin - _maragin_x;
    CGRect current_rect;
    NSMutableArray * tempSelArr = [[NSMutableArray alloc] init];
    UIButton *button = [[UIButton alloc] init];
    button.titleLabel.font = Font_Small_Text;
    
    for (NSInteger i = 0; i < dataAr.count; ++i) {

        CGSize contentSize = _butWidth>0?CGSizeMake(_butWidth, 0):[self sizeWidthWidth:dataAr[i] font:_font maxHeight:butHeight];
            //创建按钮
            ...
            [but setTitleColor:_contentEnabColor forState:UIControlStateDisabled];
            ...
            //处理标签流
            CGFloat butWidth = contentSize.width + kRealValue(30);
            butorignX = alineButWidth + _maragin_x;
            alineButWidth = _maragin_x + butWidth + alineButWidth;
            if (alineButWidth >= self.width - _maragin) {
                butorignX = _maragin;
            ...
            UIImage *normal = [UIImage setRadius:_radius size:CGSizeMake(butWidth, _butHeight) borderColor:_norBorderColor borderWidth:_borderWidth backgroundColor:_norColor];
            UIImage *selecte = [UIImage setRadius:_radius size:CGSizeMake(butWidth, _butHeight) borderColor:_selBorderColor borderWidth:_borderWidth backgroundColor:_selColor];
        UIImage *enabel = [UIImage setRadius:_radius size:CGSizeMake(butWidth, _butHeight) borderColor:_enabBorderColor borderWidth:_enabBorderColor?_borderWidth:0 backgroundColor:_enabColor];
            [but setBackgroundImage:normal forState:UIControlStateNormal];
            [but setBackgroundImage:selecte forState:UIControlStateSelected];
            [but setBackgroundImage:enabel forState:UIControlStateDisabled];
                ...
                if (_defaultSelectIndexArr.count > 0) {
                    ...
                    if (selIndexArr.count > 0) {
                        ...
                        if (i == [indexNumber integerValue]) {
                            ...
                            [tempSelArr addObject:valueStr];
                            //but.backgroundColor = _selColor;
                        }else if (_isSingle){//如果是单选，把其他选项置为不可用
                            but.enabled = NO;
                        }
                    }
                }else{
...
- (void)setDefaultEnableIndexArr:(NSArray<NSArray *> *)defaultEnableIndexArr ignoreArray:(NSArray *)ignoreArray{
    _defaultEnableIndexArr = defaultEnableIndexArr;
    //判断是否有数组
    for (NSInteger i = 0; i<_contetntArr.count; i++) {
        //只有当忽略组数为一时才启用忽略功能
        if (ignoreArray && ignoreArray.count == 1 && [ignoreArray containsObject:@(i)]) {
            NSMutableArray *curGroupArray = _contetntArr[i];
            for (NSInteger j = 0; j<curGroupArray.count; j++) {
                //将忽略组数中的所有按钮设为可用
                UIButton *but = (UIButton *)[self.scroller viewWithTag:i * 100 + j + 1];
                if(but.enabled == NO){
                    but.enabled = YES;
                }
            }
            ...
            for (NSInteger j = 0; j<curGroupArray.count; j++) {
                ...
                //but.enabled = [defaultEnableIndexArr[i] containsObject:@(j)];
                //如果包含设置为可用
                if ([defaultEnableIndexArr[i] containsObject:@(j)] ) {
                    but.enabled = YES;
                }else{
                    but.selected = NO;
                    but.enabled = NO;
...
- (void)setDefaultEnableIndexArr:(NSArray *)defaultEnableIndexArr{
    [self setDefaultEnableIndexArr:defaultEnableIndexArr ignoreArray:nil];
}
...
- (UIScrollView *)scroller{
    if (!_scroller) {
        ...
        _scroller.backgroundColor = _bgColor;
